...
class FCN16_3D(nn.Module):
    ...
    def __init__(self, n_filters, kernel_conv=3, input_channels=1):
        ...
        self.conv1_2 = nn.Conv3d(self.n_filters, self.n_filters,
                                 self.k_conv, padding=1)
        self.relu1_2 = nn.ReLU(inplace=True)
        self.pool1 = nn.MaxPool3d(2, stride=2, ceil_mode=True)  # 1/2

        # conv2
        self.conv2_1 = nn.Conv3d(self.n_filters,
                                 self.n_filters*2, self.k_conv, padding=1)
        self.relu2_1 = nn.ReLU(inplace=True)
        self.conv2_2 = nn.Conv3d(self.n_filters*2, self.n_filters*2,
                                 self.k_conv, padding=1)
        self.relu2_2 = nn.ReLU(inplace=True)
        self.pool2 = nn.MaxPool3d(2, stride=2, ceil_mode=True)  # 1/4

        # conv3
        self.conv3_1 = nn.Conv3d(self.n_filters*2,
                                 self.n_filters*4, self.k_conv, padding=1)
        self.relu3_1 = nn.ReLU(inplace=True)
        self.conv3_2 = nn.Conv3d(self.n_filters*4, self.n_filters*4,
                                 self.k_conv, padding=1)
        self.relu3_2 = nn.ReLU(inplace=True)
        self.conv3_3 = nn.Conv3d(self.n_filters*4, self.n_filters*4,
                                 self.k_conv, padding=1)
        self.relu3_3 = nn.ReLU(inplace=True)
        self.pool3 = nn.MaxPool3d(2, stride=2, ceil_mode=True)  # 1/8

        # conv4
        self.conv4_1 = nn.Conv3d(self.n_filters*4,
                                 self.n_filters*8, self.k_conv, padding=1)
        self.relu4_1 = nn.ReLU(inplace=True)
        self.conv4_2 = nn.Conv3d(self.n_filters*8, self.n_filters*8,
                                 self.k_conv, padding=1)
        self.relu4_2 = nn.ReLU(inplace=True)
        self.conv4_3 = nn.Conv3d(self.n_filters*8, self.n_filters*8,
                                 self.k_conv, padding=1)
        self.relu4_3 = nn.ReLU(inplace=True)
        self.pool4 = nn.MaxPool3d(2, stride=2, ceil_mode=True)  # 1/16

        # conv5
        self.conv5_1 = nn.Conv3d(self.n_filters*8,
                                 self.n_filters*8, self.k_conv, padding=1)
        self.relu5_1 = nn.ReLU(inplace=True)
        self.conv5_2 = nn.Conv3d(self.n_filters*8, self.n_filters*8,
                                 self.k_conv, padding=1)
        self.relu5_2 = nn.ReLU(inplace=True)
        self.conv5_3 = nn.Conv3d(self.n_filters*8, self.n_filters*8,
                                 self.k_conv, padding=1)
        ...
        self.fc7 = nn.Conv3d(self.n_filters*64, self.n_filters*64, 1)
